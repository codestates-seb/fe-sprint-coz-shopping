    ...
    return (
        <>
            ...
            <ProductItemContainer onClick={modalPopUp}>
                ...
                <div>
                    <ProductContent>
                        <ProductTitle>{data.type === "Category" 
                            ? `# ${data.title}` : data.type === "Brand" 
                            ? `${data.brand_name}` 
                            : `${data.title}`}
                        </ProductTitle>
                        {data.type === "Product" ? <ProductDiscount>{data.discountPercentage} %</ProductDiscount> 
                        : data.type === "Brand" ? <ProductTitle>관심고객수</ProductTitle> 
                        : null}
                    </ProductContent>
                    <ProductContent>
                        <ProductSubtitle>{data.type === "Exibition" ? `${data.sub_title}` : null}</ProductSubtitle>
                        <ProductNum>
                            {data.type === "Product" ? `${Number(data.price).toLocaleString()} 원` 
                            : data.type === "Brand" ? `${Number(data.follower).toLocaleString()} 명` 
                            : null}
                        </ProductNum>
                    </ProductContent>
                </div>
